  ...
  sidebarLayout(
    sidebarPanel(
      tabsetPanel(
        id = "yearInputTabs",
        tabPanel("Year 1", 
                 sliderInput("percentageOfUsersY1", "Percentage of Total Visitors Year 1:", min = 0, max = 5, value = 1, step = 0.1),
                 sliderInput("conversionRateY1", "Conversion Rate for Explorers Year 1 (%):", min = 0, max = 100, value = 2),
                 sliderInput("bUsersY1", "Number of Businesses Year 1:", min = 0, max = 500, value = 1),
                 numericInput("aFeeY1","Monthly Fee For Travelers Year 1 (€):", value = 2.99 ),
                 numericInput("bFeeY1", "Monthly Fee for Businesses Year 1 (€):", value = 20),
                 numericInput("numCitiesY1", "Number of Cities Year 1:", value = 1),
                 numericInput("totalCostY1", "Total Cost Year 1 (€):", value = 357555)
        ),
        tabPanel("Year 2", 
                 sliderInput("percentageOfUsersY2", "Percentage of Total Visitors Year 2:", min = 0, max = 5, value = 1, step = 0.1),
                 ...
                 numericInput("aFeeY2","Monthly Fee For Travelers Year 2 (€):", value = 2.99 ),
                 numericInput("bFeeY2", "Monthly Fee for Businesses Year 2 (€):", value = 20),
                 numericInput("numCitiesY2", "Number of Cities Year 2:", value = 15),
                 numericInput("totalCostY2", "Total Cost Year 2 (€):", value = 351565)
        ),
        tabPanel("Year 3", 
                 sliderInput("percentageOfUsersY3", "Percentage of Total Visitors Year 3", min = 0, max = 5, value = 1, step = 0.1),
                 sliderInput("conversionRateY3", "Conversion Rate for Explorers Year 3 (%):", min = 0, max = 100, value = 2),
                 sliderInput("bUsersY3", "Number of Businesses Year 3:", min = 0, max = 500, value = 1),
                 numericInput("aFeeY3","Monthly Fee For Travelers Year 3 (€):", value = 3.99 ),
                 numericInput("bFeeY3", "Monthly Fee for Businesses Year 3 (€):", value = 20),
                 numericInput("numCitiesY3", "Number of Cities Year 3:", value = 40),
                 numericInput("totalCostY3", "Total Cost Year 3 (€):", value = 412775)
        ),
        tabPanel("Year 4", 
                 sliderInput("percentageOfUsersY4", "Percentage of Total Visitors Year 4", min = 0, max = 5, value = 1, step = 0.1),
                 sliderInput("conversionRateY4", "Conversion Rate for Explorers Year 4 (%):", min = 0, max = 100, value = 2),
                 sliderInput("bUsersY4", "Number of Businesses Year 4:", min = 0, max = 500, value = 1),
                 numericInput("aFeeY4","Monthly Fee For Travelers Year 4 (€):", value = 3.99 ),
                 numericInput("bFeeY4", "Monthly Fee for Businesses Year 4 (€):", value = 20),
...
server <- function(input, output) {
  ...
  renderRevenuePlotForYear <- function(year) {
    renderPlot({
      ...
      addRevenue <- 2200000 * 0.01 * 0.2
      numCities <- input[[paste0("numCitiesY", year)]]
      totalCost <- input[[paste0("totalCostY", year)]]
      #note: i am using 2.2 million here - that is excluding the locals 
      aUsers <- 2200000 * (percentageOfUsers / 100) * (conversionRate / 100)
      aRevenue <- round(aUsers * aFee * 12) * numCities
      bRevenue <- round(bUsers * bFee * 12) * numCities
      adRevenue <- round(addRevenue) * numCities
  ...
  output$profitPlot <- renderPlot({
    ...
    for (year in 1:4) {  
      ...
      addRevenuePercentage <- 2  
      percentageOfUsersDecimal <- input[[paste0("percentageOfUsersY", year)]] / 100
      addRevenue <- 2200000 * percentageOfUsersDecimal * (addRevenuePercentage / 100)
      adRevenue <- round(addRevenue) * numCities
      

      aUsers <- 2200000 * (input[[paste0("percentageOfUsersY", year)]] / 100) * (input[[paste0("conversionRateY", year)]] / 100)
      aRevenue <- round(aUsers * input[[paste0("aFeeY", year)]] * 12) * numCities
      bRevenue <- round(input[[paste0("bUsersY", year)]] * input[[paste0("bFeeY", year)]] * 12) * numCities
    ...
    financialData <- data.frame(
      ...
      Amount = c(-totalCostAllYears, totalRevenueAllYears, netProfit), 
      Color = c("orange2", "deepskyblue", netProfitColor) 
    )
